...
#ifndef _MX_CH59X_GPIO_H_
#define _MX_CH59X_GPIO_H_
...
/* GPIO原子操作宏，如果需要在中断使用GPIO，则程序内的打开中断后的GPIO操作应该改为原子操作 */

/* 原子操作设置引脚电平为高 */
#define GPIO_ATOMIC_SETBITS(PORT, PIN)                  do{__MX_VOID_AMOOR_W(&(R32_Px_OUT(PORT)), PIN);}while(0)

/* 原子操作设置引脚电平为低 */
#define GPIO_ATOMIC_RESETBITS(PORT, PIN)                do{__MX_VOID_AMOAND_W(&(R32_Px_OUT(PORT)), (~(PIN)));}while(0)

/* 原子操作翻转 GPIO电平 */
#define GPIO_ATOMIC_INVERSEBITS(PORT, PIN)              do{__MX_VOID_AMOXOR_W(&(R32_Px_OUT(PORT)), PIN);}while(0)

/* 原子操作设置GPIO上拉 - R32_Px_PU */
#define GPIO_ATOMIC_SET_PU(PORT, PIN)                   do{__MX_VOID_AMOOR_W(&(R32_Px_PU(PORT)), (PIN));}while(0)

/* 原子操作取消GPIO上拉 - R32_Px_PU*/
#define GPIO_ATOMIC_RESET_PU(PORT, PIN)                 do{__MX_VOID_AMOAND_W(&(R32_Px_PU(PORT)), (~(PIN)));}while(0)

/* 原子操作GPIO方向输出 - R32_Px_DIR */
#define GPIO_ATOMIC_SET_DIR(PORT, PIN)                  do{__MX_VOID_AMOOR_W(&(R32_Px_DIR(PORT)), (PIN));}while(0)

/* 原子操作GPIO方向输入 - R32_Px_DIR */
#define GPIO_ATOMIC_RESET_DIR(PORT, PIN)                do{__MX_VOID_AMOAND_W(&(R32_Px_DIR(PORT)), (~(PIN)));}while(0)

/* 原子操作GPIO输入下拉，输出20ma - R32_Px_DIR */
#define GPIO_ATOMIC_SET_PD_DRV(PORT, PIN)               do{__MX_VOID_AMOOR_W(&(R32_Px_PD_DRV(PORT)), (PIN));}while(0)

/* 原子操作GPIO输入不下拉，输出5ma - R32_Px_DIR */
#define GPIO_ATOMIC_RESET_PD_DRV(PORT, PIN)             do{__MX_VOID_AMOAND_W(&(R32_Px_PD_DRV(PORT)), (~(PIN)));}while(0)

/* 原子操作设置GPIO为浮空输入 */
#define GPIO_ATOMIC_MODECFG_IN_FLOATING(PORT, PIN)      do{GPIO_ATOMIC_RESET_PD_DRV(PORT, PIN);GPIO_ATOMIC_RESET_PU(PORT, PIN);GPIO_ATOMIC_RESET_DIR(PORT, PIN);}while(0)

/* 原子操作设置GPIO为上拉输入 */
#define GPIO_ATOMIC_MODECFG_IN_PU(PORT, PIN)            do{GPIO_ATOMIC_RESET_PD_DRV(PORT, PIN);GPIO_ATOMIC_SET_PU(PORT, PIN);GPIO_ATOMIC_RESET_DIR(PORT, PIN);}while(0)

/* 原子操作设置GPIO为下拉输入 */
#define GPIO_ATOMIC_MODECFG_IN_PD(PORT, PIN)            do{GPIO_ATOMIC_SET_PD_DRV(PORT, PIN);GPIO_ATOMIC_RESET_PU(PORT, PIN);GPIO_ATOMIC_RESET_DIR(PORT, PIN);}while(0)

/* 原子操作设置GPIO为5ma输出 */
#define GPIO_ATOMIC_MODECFG_OUT_PP_5MA(PORT, PIN)       do{GPIO_ATOMIC_RESET_PD_DRV(PORT, PIN);GPIO_ATOMIC_SET_DIR(PORT, PIN);}while(0)

/* 原子操作设置GPIO为20ma输出 */
#define GPIO_ATOMIC_MODECFG_OUT_PP_20MA(PORT, PIN)      do{GPIO_ATOMIC_SET_PD_DRV(PORT, PIN);GPIO_ATOMIC_SET_DIR(PORT, PIN);}while(0)

#endif /* _MX_CH59X_GPIO_H_ */
